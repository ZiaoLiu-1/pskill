...
func (c *Client) Search(query string, limit int) ([]SkillResult, error) {
	...
	reqURL := fmt.Sprintf("%s/api/search?q=%s&limit=%d", c.baseURL, url.QueryEscape(query), limit)
...
func (c *Client) Trending(limit int, rangeKey, category string) ([]SkillResult, error) {
	...
	reqURL := fmt.Sprintf("%s/api/trending?limit=%d&range=%s&category=%s", c.baseURL, limit, url.QueryEscape(rangeKey), url.QueryEscape(category))
	req, _ := http.NewRequest(http.MethodGet, reqURL, nil)
	resp, err := c.http.Do(req)
	if err != nil {
		out := []SkillResult{
...
func (c *Client) DownloadSkill(name, destination string) error {
	...
	reqURL := fmt.Sprintf("%s/api/skills/%s/download", c.baseURL, url.PathEscape(name))
	req, _ := http.NewRequest(http.MethodGet, reqURL, nil)
	resp, err := c.http.Do(req)
	if err != nil {
